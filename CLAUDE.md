...
**Single-file vanilla JS SPA** — `index.html` (~7861 lines) contains all HTML, CSS, and JS. No build step, no framework, no npm. Deployed as a static site on Vercel. Backend is Supabase (PostgreSQL + Auth + Storage + Realtime + Edge Functions).
...
**Modify pricing:** Update `calcJobPrice()` in `index.html` (line ~946) AND `booking-webhook/index.ts` (line ~39) — they must stay in sync.

**Modify checklist template:** Update `STANDARD_CHECKLIST` in `index.html` (line ~979) AND `booking-webhook/index.ts` (line ~63) — keep in sync.
