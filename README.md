...
- **Calendar** — Month view of all scheduled jobs; export upcoming 90-day schedule as `.ics` (Google Calendar, Apple Calendar, Outlook)
...
- **Checklists** — Standard cleaning checklist per job (includes laundry: strip, wash, dry, replace linens, fold and return towels)
...
- Laundry required on every clean: Strip dirty linens → Wash linens → Dry linens → Replace linens on all beds → Fold towels → Place clean towels in bathrooms
...
```
/
├── index.html                                       # Entire SPA (~7928 lines, vanilla JS)
...
`SUPABASE_URL` is automatically available in edge functions. `SUPABASE_ANON_KEY` must be set manually as a secret — it is used by quickbooks-sync, quickbooks-payment-check, quickbooks-oauth, and invite-user to verify user sessions, **and by the pg_cron scheduled job for `mark-overdue-invoices`** (the cron job sends the anon key as its Bearer token so the function can authenticate the scheduled call). If this secret is missing, the daily cron silently returns 401 and stops marking invoices overdue.
...
3. Creates a full 34-item checklist (including laundry tasks)
4. Logs the activity
...
| Trigger | Auto Action |
|---------|------------|
| Booking confirmed (webhook or CRM) | Creates cleaning job for checkout date + 34-item checklist |
...
- **Pay rate confidentiality**: The Team page (visible to all authenticated users, not just admins) no longer fetches or renders `pay_rate` for non-admin viewers — previously every field contractor's hourly rate was visible to every other contractor on that page. The job detail view and the assignment-tab employee picker/list had the same gap (they embedded `employees(*)`/fetched `employees.select('*')` purely to show a name, shipping every assigned contractor's pay rate to any viewer's network tab on every job load); those queries are now scoped to the columns actually rendered. `showEditContractorModal` also gained an explicit admin check so the edit-contractor prefill (which does need `pay_rate`) can't be triggered by a non-admin calling the function directly. The underlying `employees_select` RLS policy is `USING (true)` for all columns — Postgres RLS can't filter columns — so a DB-level column-masking view (`employees_masked`) was added to the schema. The view returns `NULL` for `pay_rate` when the caller is not an admin (checked via `auth.uid()` against `profiles.role`). Non-admin queries in `renderTeam()` now target `employees_masked` instead of the `employees` table directly, closing the raw REST API bypass (`/rest/v1/employees?select=pay_rate` returns `null` for non-admins).
...
- **Schema line count**: `supabase-schema.sql` is ~1,366 lines; `index.html` is ~7928 lines.
...
- **`mark-overdue-invoices` edge function used UTC date instead of America/Chicago**: `new Date().toISOString().slice(0,10)` truncates to the UTC calendar date — for Abilene, TX (UTC-5/-6) this rolls a day forward from 6 PM local time onward, so the admin-triggered "Run Now" button on the Integrations page could mark invoices due today as overdue up to 6 hours early. The scheduled cron fires at 6 AM UTC (midnight CT), where the UTC and CT calendar dates happen to coincide, so that path was silently unaffected. Replaced with `Intl.DateTimeFormat('en-CA', { timeZone: 'America/Chicago' }).format(new Date())`, matching the front-end `localToday()` fix already applied to ~12 client-side call sites.
- **Messages page was missing deletion despite the RLS policy supporting it**: the `messages` RLS already allows authors to delete their own messages and admins to delete any message (`messages_delete` policy), but `_renderMessageBubbles()` never rendered a delete control — the UI gap made the schema guarantee unreachable. A hover-reveal ✕ button now appears on each message bubble for its author and for admins, backed by `deleteMessage()` which calls `confirmDialog()` before deleting and refreshes the list, matching the delete-with-confirm pattern used on every other entity in this file.

- **Messages page was also missing editing despite the RLS policy supporting it**: the `messages_update` policy (`USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)`) allows the original author to update their own message body, but no edit UI existed — the policy was unreachable from the browser. A hover-reveal ✎ button now appears on the author's own message bubbles (not on others' messages — the RLS restricts update to the author only), opening `showEditMessageModal()` with the existing body pre-filled. `saveEditMessage()` calls `.update({body}).eq('id', msgId).eq('user_id', currentUser.id)` (the `.eq('user_id')` is a client-side ownership guard that mirrors the RLS `USING` clause, ensuring any console-abuse attempt gets a clean JS-layer rejection before the DB even sees the request) and refreshes the list on success, matching the delete-and-refresh pattern already used by `deleteMessage()`.
- **`calToday()` used UTC year/month after the init fix only updated the module-level `calYear`/`calMonth` vars**: the most recent commit pinned the calendar's initial month/year to `America/Chicago` via `Intl.DateTimeFormat`, but the "Today" nav button (`calToday()`) still called `new Date().getFullYear()`/`new Date().getMonth()` — the UTC-based form — so clicking "Today" in the evening (after 6 PM Central) would silently jump to the UTC date (next calendar month or year at month boundaries) rather than Chicago's current month. Fixed to use the same `Intl.DateTimeFormat` pattern as the init.
- **`nextInvoiceNumber()` used UTC year for the invoice number prefix**: the auto-sequence that generates `RNV-YYYY-NNNN` invoice numbers called `new Date().getFullYear()`, which returns the UTC year — for Abilene, TX this rolls forward to the next year 6 hours early on December 31st, producing invoices numbered e.g. `RNV-2027-0001` while the calendar still shows December 31, 2026. Fixed with the same `Intl.DateTimeFormat` pattern, matching every other date computation. (The placeholder text in the Edit Invoice modal's invoice-number field has the same cosmetic issue and was left as-is — it's a display hint only, never auto-filled.)
- **Edit-message button was broken for any message body containing a double-quote, and had a latent stored-XSS vector**: `_renderMessageBubbles()` built the edit button's `onclick` attribute with `JSON.stringify(body)` embedded directly inside the HTML attribute value. Any message body containing a `"` terminated the attribute at that point, silently making the button non-functional for that message, and a crafted body like `"); alert('xss');//` would execute arbitrary JS. Replaced with `data-msgid`/`data-body` attributes (values HTML-encoded through `esc()`) read back at runtime via `this.dataset` — the same safe pattern used everywhere else in the file for dynamic event handlers. Also updated a stale comment in `supabase-schema.sql` that claimed the app never edits messages.
- **iCal export silently dropped `LOCATION` from all employee calendar events**: `exportCalendarICS()` scoped the non-admin query to `properties(name)` only, omitting `address` and `city`. The ICS builder then set `loc = ''` for every event since `j.properties?.address` and `j.properties?.city` were both `undefined`, and the RFC 5545 `LOCATION` field was gated behind `if (loc)` — so employees downloading their `.ics` schedule got events with no location, without any warning. The non-admin column list is now `properties(name,address,city)`, matching what the ICS builder actually uses. (Property address is not a sensitive field; only pricing and pay rate are gated behind admin-only column selects.)
- **Admin Settings had no way to update a user's email address**: `showEditUserModal()` displayed the current email as read-only text, with no input field. The `saveEditUser()` function called only `update_profile` (name/role) and `update_password`, leaving email permanently locked once a user was created. Added an editable email field to the modal (with `data-original` attribute for change detection) and a new `update_email` action in the `invite-user` edge function that calls `adminClient.auth.admin.updateUserById(userId, { email })` (service-role-level change, no user verification step) then syncs the new address back to `profiles.email` (which the `handle_new_user` trigger doesn't update on auth-level email changes). Email is validated client-side before the call, and the update is skipped entirely when the field is unchanged — preserving the existing two-call pattern (`update_profile` then optional `update_password`) for the no-email-change case.
- **Three error paths had `console.warn`-only handling with no user-visible feedback**: (1) `showJobDetail()` logged checklist and invoice fetch failures silently — the job detail panel opened but showed empty checklist and invoice sections with no explanation; toasts are now shown alongside the existing log lines, matching the already-correct media-fetch-failure path in the same function. (2) `autoCreateJobFromBooking()` logged a checklist-creation failure silently — the job was created but the crew would later find no checklist to complete, with no UI signal that one needed to be generated manually; a warning toast is now shown, matching the same warning already present in the UI-triggered `createJob()` path. (3) `deleteInvoice()` fell through to the delete confirmation when the invoice status pre-fetch failed, silently bypassing the "This invoice is already PAID — are you sure?" gate; it now returns early with an error toast ("Could not verify invoice status — please try again") so the admin can retry once the transient error clears rather than unknowingly deleting a paid invoice without the special confirmation.
