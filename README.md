...
```
/
├── index.html                                       # Entire SPA (~5,224 lines, vanilla JS)
...
- **Schema line count**: `supabase-schema.sql` is ~1,156 lines; `index.html` is ~5,224 lines.
...
- **quickbooks-payment-check never marked a $0 invoice paid**: the fully-paid check was `total > 0 && balance === 0`, which excludes the case of a $0 invoice with a $0 QB balance — trivially settled, but never reachable by the `total > 0` guard. Not currently exploitable (every invoice-creation path in the UI rejects amounts ≤ 0), but `invoices.amount` itself only has a `>= 0` constraint, so it was a latent gap for any future code path. Simplified to `balance === 0`, which correctly covers both cases.
